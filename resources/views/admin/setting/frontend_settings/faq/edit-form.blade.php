<div class="p-sm-25 p-15">
    ...
    <form class="ajax reset" action="{{ route('admin.setting.frontend.faq.update', $faq->id) }}" method="post"
        ...
        <div class="row rg-24">
            <div class="col-12">
                <label class="zForm-label" for="title">{{ __('Question') }} <span class="text-danger">*</span></label>
                <input class="zForm-control" type="text" name="question" placeholder="{{ __('Question') }}"
                       value="{{$faq->question}}">
            </div>

            <div class="col-12">
                <label class="zForm-label" for="description">{{ __('Answer') }} <span
                        class="text-danger">*</span></label>
                <textarea class="zForm-control" name="answer" rows="5" id="">{{$faq->answer}}</textarea>
            </div>
            <div class="col-12">
