<div class="p-sm-25 p-15">
    ...
    <form class="ajax reset" action="{{ route('admin.tickets.tag-store') }}" method="post" data-handler="commonResponseForModal">
        @csrf

            <input type="hidden" name="id" value="{{$singleData->id}}">
            <div class="row rg-24">
                <div class="col-12">
                    <label class="zForm-label" for="title">{{ __('Name') }} <span class="text-danger">*</span></label>
                    <input type="text" name="name" required class="zForm-control" placeholder="{{ __('Name') }}" value="{{$singleData->name}}">
                </div>
                ...
                <div class="col-12">
                    ...
                    <select name="status" class="form-control">
                        ...
                        <option value="0" {{$singleData->status == 0?'selected':''}}>{{ __('Deactivate') }}</option>
                    </select>
                </div>
            </div>
