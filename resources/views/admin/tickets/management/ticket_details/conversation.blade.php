<div class="p-sm-25 p-15 bd-one bd-c-stroke bd-ra-10 bg-white mb-24">
    <div class="alertMassas collision-detector-alert d-none bd-ra-10 overflow-hidden mb-20">
        <p><svg xmlns="http://www.w3.org/2000/svg" width="14" height="12" viewBox="0 0 14 12" fill="none">
                <path
    ...
    <div class="view-post-reply">
        <div class="d-flex justify-content-between align-items-center bd-b-one bd-c-stroke pb-17 mb-17">
                        ...
                        <span class="iconNotifi ">
                            ...
                            <div class="editPart aiReplayBtu popup top-down">
                                ...
                                <div class="coustomPart generate-ai-replay" data-id="{{$ticketData->id}}">
                                    ...
                                    <div class="spinner-border text-primary d-none w-20 h-20" role="status">
                                ...
                                <div class="reply-list">
                                    @foreach(aiReplyList($ticketData->id) as $reply)
                                        <div class="coustomPart-new">
                                            <div class="show-read-more reply-result" data-result="{{$reply->ai_replay_text}}">{{$reply->ai_replay_text}}</div>
                    ...
                    @endif
                    <button class="ticketNote py-10 px-26 d-flex align-items-center g-12 fs-15 fw-600 lh-25 text-white bg-main-color bd-one bd-c-main-color bd-ra-8" data-ticke_id="{{$ticketData->id}}">
                        {{__("+ Ticket Note")}}
                    </button>
        </div>
        ...
        <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade show active" id="nav-home2" role="tabpanel" aria-labelledby="nav-home-tab2"
                 tabindex="0">
                <form action="{{route('admin.conversations.conversationStore',encrypt($ticketData->id))}}" method="post" class="form-horizontal" enctype="multipart/form-data">
                    @csrf
                    <div class="summernote-reply sf-ticket-details-reply">
                        <div class="d-flex justify-content-between align-items-start flex-wrap g-20 pb-20">
                            ...
                            <div class="ticket-status-change">
                                <div class="d-flex align-items-center g-10 flex-wrap">
                                    <input type="hidden" id="ticketStatusChangeRoute" value="{{ route('admin.tickets.ticketStatusUpdate') }}">
                                    <div class=" checkrowed zForm-wrap-checkbox-2" onclick="changeTicketStatus({{$ticketData->id}},{{STATUS_ON_HOLD}},{{$ticketData->status}})">
                                        <input type="radio" class="form-check-input status{{STATUS_ON_HOLD}}"
                                               @if($ticketData->status==STATUS_ON_HOLD) checked="checked"
                                               @endif  id="onHold"
                                               name="ticket_status">
                                        <label class="onHoldColor m-0" for="onHold">{{ __('On-Hold') }}</label>

                                    </div>
                                    <div class=" checkrowed zForm-wrap-checkbox-2"
                                         onclick="changeTicketStatus({{$ticketData->id}},{{STATUS_CLOSED}},{{$ticketData->status}})">
                                        <input type="radio" class="form-check-input status{{STATUS_CLOSED}}"
                                               @if($ticketData->status==STATUS_CLOSED) checked="checked"
                                               @endif id="closed"
                                               name="ticket_status">
                                        <label class="closedColor m-0" for="closed">{{ __('Closed') }}</label>

                                    </div>
                                    <div class=" checkrowed zForm-wrap-checkbox-2"
                                         onclick="changeTicketStatus({{$ticketData->id}},{{STATUS_SUSPENDED}},{{$ticketData->status}})">
                                        <input type="radio" class="form-check-input status{{STATUS_SUSPENDED}}"
                                               @if($ticketData->status==STATUS_SUSPENDED) checked="checked"
                                               @endif id="suspend"
                                               name="ticket_status">
                                        <label class="suspendColor m-0" for="suspend"
                                        >Suspend</label>

                                    </div>
                                    <div class=" checkrowed zForm-wrap-checkbox-2"
                                         onclick="changeTicketStatus({{$ticketData->id}},{{STATUS_INPROGRESS}},{{$ticketData->status}})">
                                        <input type="radio" class="form-check-input status{{STATUS_INPROGRESS}}"
                                               @if($ticketData->status==STATUS_INPROGRESS) checked="checked"
                                               @endif id="processing"
                                               name="ticket_status">
                                        <label class="processingColor m-0"
                                               for="processing">{{ __('Processing') }}</label>

                                    </div>
                                    <div class=" checkrowed zForm-wrap-checkbox-2"
                                         onclick="changeTicketStatus({{$ticketData->id}},{{STATUS_RESOLVED}},{{$ticketData->status}})">
                                        <input type="radio" class="form-check-input status{{STATUS_RESOLVED}}"
                                               @if($ticketData->status==STATUS_RESOLVED) checked="checked"
                                               @endif id="solved"
                                               name="ticket_status">
                                        <label class="solvedColor m-0 me-0" for="solved">{{ __('Solved') }}</label>

                                    </div>
                                </div>
                            </div>
                            <!-- Instant Reply -->
                            <div class="flex-shrink-0 d-flex align-items-center g-10 flex-wrap">
                                ...
                                @endif
                                <div class="dropdown">
                                    <button type="button" class="dropdown-toggle bg-transparent border-0 p-0 d-flex align-items-center g-10" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                        <img src="{{ asset('agent/assets/images/akar-icons.png')}}" alt="">
                                        {{ __('Instant Reply') }}
                                    </button>
                                    <div class="dropdown-menu dropdown-menu-end dropdownItem-three">
                                        <div class="search-one mb-24">
                                            <input type="text" placeholder="{{ __('Search') }}" id="instantReplySearch">
                                            <button class="icon"><img src="{{asset('main_assets/images/icon/search.svg')}}" alt=""></button>
                                        </div>
                                        <ul>
                                            @foreach($instantMessageData as $istMsg)
                                                <li class="view-user-list clock-instant d-flex justify-content-between align-items-center g-10">
                                                    <div class="align-items-start d-flex gap-2 w-100">
                                                        ...
                                                        <a href="#" onclick="loadInstantMessage({{$istMsg->id}})" data-target="{{$istMsg->id}}"><p>{{$istMsg->title}}</p></a>
                                                    </div>
                                                    <div class="d-flex gap-2">
                                                        <button type="button" class="border-0 p-0 bg-transparent font-11 text-secondary" onclick="editInstantMessage('{{$istMsg->id}}', '{{$istMsg->title}}', '{{$istMsg->message}}')">
                                                            <i class="fa-solid fa-pen"></i></button>
                                                        <button type="button" class="border-0 p-0 bg-transparent font-11 text-danger" onclick="deleteItem('{{route('admin.conversations.instant.message.delete', $istMsg->id)}}','',true)">
                                        ...
                                        <button type="button" class="ticket-btu-com add-ins-reply py-10 px-26 d-flex align-items-center g-12 fs-15 fw-600 lh-25 text-white bg-main-color bd-one bd-c-main-color bd-ra-8 mt-25 w-100 text-center" data-bs-toggle="modal" data-bs-target="#InstantModal" data-bs-whatever="@mdo">{{__('Add Instant Reply')}}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="pb-20">
                            <textarea class="summernoteOne" name="conversation_details" id="conversation_details"></textarea>
                        </div>
                    </div>
                        ...
                        <div class="ticket-upload-btn-box">
                            ...
                            <div class="d-flex justify-content-between align-items-center flex-wrap g-20">
                                <div class="choose-file-border file-upload-one">
                                    ...
                                    <input type="file" multiple="" data-max_length="20" id="ticket-upload-img" name="file[]" class="ticket-img-input d-none">
                                </div>
                                <button type="submit" @if($ticketData->status==STATUS_CLOSED) disabled
                                        @endif class="m-0 fs-15 border-0 fw-500 lh-25 text-white py-10 px-26 bg-main-color bd-ra-12">{{__('Reply Now')}}</button>
                            </div>
                        </div>
