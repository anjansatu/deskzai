@if(isset($envato->envato_expired_on) && $envato->envato_expired_on == STATUS_ACTIVE)
    <div
        class="d-flex justify-content-between align-items-center g-10 flex-wrap p-sm-25 p-15 bd-ra-10 mb-24 {{($envatoData == null) || (isset($envatoDayCount) && $envatoDayCount == 0) ?'purchase-red':'purchase-green'}} ">

        <h5 class="fs-18 fw-600 lh-22 text-title-black">{{isset($envatoData->item->name)?$envatoData->item->name:''}}</h5>
        <button
            class="border-0 bg-main-color py-8 px-26 bd-ra-8 fs-15 fw-600 lh-25 text-white d-inline-flex align-items-center g-10"
...
<div class="p-sm-25 p-15 bd-one bd-c-stroke bd-ra-10 bg-white mb-24">
    <div class="view-assign">
        <div
            class="d-flex justify-content-between align-items-start align-items-sm-center g-20 bd-b-one bd-c-stroke pb-17 mb-17">
            <div class="d-flex align-items-center flex-wrap flex-sm-nowrap g-20 ticket-assignTag-dropdown">
                <!-- Assignments -->
                <div class="dropdown">
                    <button
                        class="dropdown-toggle assignmentsagent bg-transparent py-8 px-12 bd-one bd-c-stroke bd-ra-8 fs-15 fw-500 lh-25 text-para-text d-inline-flex align-items-center g-10"
                    ...
                    <div class="dropdown-menu dropdown-menu-end dropdownItem-three" aria-labelledby="dropdownMenuLink">
                        ...
                        <div class="search-one mb-10">
                            ...
                            <button class="icon"><img src="{{asset('main_assets/images/icon/search.svg')}}" alt="">
                            </button>
                        </div>
                        ...
                        <ul class="m-0 p-0">
                            @foreach ($userList as $user)
                                <li class="view-user-list d-flex align-items-center g-9">
                ...
                <div class="dropdown">
                    <button
                        class="dropdown-toggle taggingagent bg-transparent py-8 px-12 bd-one bd-c-stroke bd-ra-8 fs-15 fw-500 lh-25 text-para-text d-inline-flex align-items-center g-10"
                    ...
                    <div class="dropdown-menu dropdown-menu-end dropdownItem-three" aria-labelledby="dropdownMenuLink1">
                        ...
                        <input type="hidden" id="addTagRoute" value="{{ route('admin.tickets.addTicketTags') }}">
                        <ul class="m-0 p-0">
                ...
                <div class="ticketUserImages">
                    @foreach ($userList as $user)
                        @if(in_array($user->id,$ticketUserData))
                            ...
                            @else
                                <img title="{{$user->name.'('.$user->email.')'}}" class=""
                                     src={{ getFileUrl($user->image) }} alt="img">
                            @endif
                        @endif
                    @endforeach
                </div>
            ...
            <div class="dropdown">
                <button
                    class="dropdown-toggle w-30 h-30 bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center fs-13 text-para-text"
                    ...
                    <li>
                        <a href="{{route('admin.tickets.my-ticket-history', ['id'=>$ticketCreatorData->id])}}"
                           class="d-flex align-items-center cg-8">
                            <p class="fs-14 fw-500 lh-17 text-para-text">{{__('All Tickets')}}</p>
                        </a>
                    </li>
                </ul>
            </div>
        ...
        <div class="ticket-details">
            <h3 class="fs-18 fw-600 lh-22 text-title-black pb-8">#{{$ticketData->tracking_no??""}}
                -{{$ticketData->ticket_title??""}}</h3>
                    ...
                    @foreach(json_decode($ticketData->file_id) as $key=>$fileData)
                        @if(in_array(getFileType($fileData), ['image/jpeg','image/jpg','image/png','image/webp']))
                            <a class="test-popup-link w-40 h-40 bd-one bd-c-stroke bd-ra-4"
                               href="{{ getFileUrl($fileData) }}"><img
                                    src="{{ getFileUrl($fileData) }}" alt="" class="w-100 h-100 object-fit-cover"></a>
                        @else
            ...
            <div class="p-sm-25 p-15">
                ...
                @else
                    ...
                    <ul class="zList-pb-16">
                        <li class="d-flex align-items-center g-20">
                        ...
                        <li>
                            <h5 class="fs-14 fw-500 lh-17 text-title-black">{{__("Item Name")}}:</h5>
                            <p class="fs-12 fw-400 lh-17 text-para-text">{{isset($envatoData->item->name)?$envatoData->item->name:''}}</p>
                        </li>
                        <li class="d-flex align-items-center g-20">
                            <h5 class="fs-14 fw-500 lh-17 text-title-black">{{__("Client")}}:</h5>
                            <p class="fs-12 fw-400 lh-17 text-para-text">{{isset($envatoData->buyer)?$envatoData->buyer:''}}</p>
                        </li>
                        <li class="d-flex align-items-center g-20">
                            <h5 class="fs-14 fw-500 lh-17 text-title-black">{{__("Sold At")}}:</h5>
                        ...
                        <li class="d-flex align-items-center g-20">
                            <h5 class="fs-14 fw-500 lh-17 text-title-black">{{__("Support Until")}}:</h5>
                            <div
                                class="fs-12 fw-400 lh-17 text-para-text">{{isset($envatoData->supported_until)?date('d-m-Y',strtotime($envatoData->supported_until)):''}}
                                ...
                                @if(isset($envatoDayCount) && $envatoDayCount != 0)
                                    <p class="zBadge zBadge-supported">{{__("Supported")}}</p>
                                @else
                                    <p class="zBadge zBadge-expiry">{{__("License Expiry")}}</p>
                                @endif
                            </div>
                        </li>
                        <li class="d-flex align-items-center g-20">
                            <h5 class="fs-14 fw-500 lh-17 text-title-black">{{__("License")}}:</h5>
                            <p class="fs-12 fw-400 lh-17 text-para-text">{{isset($envatoData->license)?$envatoData->license:''}}</p>
                        </li>
                        <li class="d-flex align-items-center g-20">
                            <h5 class="fs-14 fw-500 lh-17 text-title-black">{{__("Count")}}:</h5>
