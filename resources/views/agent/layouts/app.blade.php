...
<body
    class="{{session()->get('dark_mood') == 1 || session()->get('dark_mood') != null?'dark':''}} {{ selectedLanguage()->rtl == 1 ? 'direction-rtl' : 'direction-ltr' }}">

<!-- Pre Loader Area start -->
@if(getOption('app_preloader_status', 0) == STATUS_ACTIVE)
    <div id="preloader">
        <div id="preloader-status"><img src="{{getFileUrl(getGeneralSettingData(getUserIdByTenant(),'app_preloader'))}}"
                                        alt="img"></div>
    </div>
@endif
<!-- Pre Loader Area End -->
