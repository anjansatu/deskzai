<div class="p-sm-25 p-15 bd-one bd-c-stroke bd-ra-10 bg-white mb-24">
    <div class="alertMassas collision-detector-alert d-none">
        <p>
            ...
            {{__("Collision detector stoped your massage")}}
        </p>
    </div>
    <div class="view-post-reply">
        <div class="d-flex justify-content-between align-items-center bd-b-one bd-c-stroke pb-17 mb-17">
            ...
            @endif
            <button
                class="ticketNote py-10 px-26 d-flex align-items-center g-12 fs-15 fw-600 lh-25 text-white bg-main-color bd-one bd-c-main-color bd-ra-8"
                data-ticke_id="{{$ticketData->id}}">
                + {{__('Ticket Note')}}
            </button>
        ...
        <form action="{{route('agent.conversations.conversationStore',encrypt($ticketData->id))}}" method="post"
            ...
            <div class="summernote-reply sf-ticket-details-reply">
                <div class="sf-ticket-statusInstantReply align-items-start align-items-lg-center align-items-xl-start align-items-xxl-center">
                    <div class="d-flex justify-content-between align-items-start flex-wrap g-20 pb-20">
                            ...
                            <div class="d-flex align-items-center g-10 flex-wrap">
                                ...
                                <div class=" checkrowed zForm-wrap-checkbox-2"
                                    ...
                                    <input type="radio" class="status{{STATUS_SUSPENDED}}"
                                           ...
                                           @endif id="suspend"
                                           name="ticket_status">
                                    <label class="suspendColor mt-1" for="suspend">Suspend</label>
                                </div>
                                <div class=" checkrowed zForm-wrap-checkbox-2"
                        ...
                        <div class="flex-shrink-0 d-flex align-items-center g-10 flex-wrap">
                            ...
                            <div class="dropdown">
                                <button
                                    class="dropdown-toggle bg-transparent border-0 p-0 d-flex align-items-center g-10"
                                ...
                                <div class="dropdown-menu dropdown-menu-end dropdownItem-three">
                                    <div class="search-one mb-24">
                                        ...
                                        <button class="icon"><img src="{{asset('main_assets/images/icon/search.svg')}}"
                                                                  alt=""></button>
                                    </div>
                                    <ul>
                                        @foreach($instantMessageData as $istMsg)
                                            <li class="view-user-list clock-instant d-flex justify-content-between align-items-center g-10">
                                                <div class="align-items-start d-flex gap-2 w-100">
                                                    ...
                                                    <input type="hidden" id="{{$istMsg->id}}"
                                                           value="{{$istMsg->message}}">
                                                    <a href="#" onclick="loadInstantMessage({{$istMsg->id}})"
                                                       data-target="{{$istMsg->id}}"><p>{{$istMsg->title}}</p></a>
                                                </div>
                                                <div class="d-flex gap-2">
                                                    <button type="button"
                                                            class="border-0 p-0 bg-transparent font-11 text-secondary"
                                                            onclick="editInstantMessage('{{$istMsg->id}}', '{{$istMsg->title}}', '{{$istMsg->message}}')">
                                                        <i class="fa-solid fa-pen"></i></button>
                                                    <button type="button"
                                                            class="border-0 p-0 bg-transparent font-11 text-danger"
                                    ...
                                    <button type="button"
                                        class="ticket-btu-com add-ins-reply py-10 px-26 d-flex align-items-center g-12 fs-15 fw-600 lh-25 text-white bg-main-color bd-one bd-c-main-color bd-ra-8 mt-25 w-100 text-center"
                                        data-bs-toggle="modal" data-bs-target="#InstantModal"
                                        data-bs-whatever="@mdo">{{__('Add Instant Reply')}}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="ticket-upload-btn-box">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap g-20">
                        <div class="choose-file-border file-upload-one">
                            ...
                            <input type="file" multiple="" data-max_length="20" id="ticket-upload-img" name="file[]"
                                   class="ticket-img-input d-none">
                        </div>
                        <button type="submit" @if($ticketData->status==STATUS_CLOSED) disabled
                                @endif class="m-0 fs-15 border-0 fw-500 lh-25 text-white py-10 px-26 bg-main-color bd-ra-12">{{__('Send Message')}}</button>
                    </div>
                </div>
