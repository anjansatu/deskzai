        ...
        return datatables($ticketData)
            ...
            ->editColumn('updated', function ($data) {
                if($data->lastConversation?->created_at){
                    $userHtml = '<div class="">
                                     <p class="fs-14 fw-500 lh-17 text-title-black pb-8">'.$data->lastConversationUser?->name.'</p>
                                    <p class="fs-12 fw-500 lh-18 text-para-text">'.Carbon::createFromFormat('Y-m-d H:i:s', $data->lastConversation?->created_at)->diffForHumans().'</p>
                                </div>';
                    return $userHtml;
                }else{
...
//            })
            ...
            ->editColumn('updated', function ($data) {
                if($data->lastConversation?->created_at){
                    $userHtml = '<div class="">
                                     <h5 class="fs-14 fw-500 lh-17 text-title-black pb-8">'.$data->lastConversationUser?->role == USER_ROLE_CUSTOMER ? 'Customer': 'Agent'.'</h5>
                                     <p class="fs-12 fw-500 lh-18 text-para-text">'.Carbon::createFromFormat('Y-m-d H:i:s', $data->lastConversation?->created_at)->diffForHumans().'</p>
                                </div>';
                    return $userHtml;
                }else{
            ...
            ->addColumn('ticket_id', function ($data) {
                if ($data->last_reply_id == null) {
                    return '<a href="' . route('admin.tickets.ticket_view', $data->id) . '" class="btn btn-outline-primary position-relative" href="">
                            ' . $data->tracking_no . '
