...
class   AgentTicketService
{
    ...
    public function list($request, $ticket_status, $customer_id = null)
    {
        ...
        if ($request->ticket_category_id && $request->ticket_category_id !== 'All') {
            $ticketData->where('tickets.category_id', $request->ticket_category_id);
        }
        ...
        return datatables($ticketData)
            ...
            ->editColumn('created_by', function ($data) {
                $userHtml = '<div class="d-flex g-10 align-items-center flex-wrap">
                                <div class="w-55 h-55 rounded-circle overflow-hidden">
                                   <img src="'.getFileUrl($data->image).'" alt="" class="w-100 h-100 object-fit-cover">
                                </div>
                                <div class="">
                                    <h5 class="fs-14 fw-500 lh-17 text-title-black pb-8">
                                        '.$data->name.'
                                    </h5>
                                     <p class="fs-12 fw-500 lh-18 text-para-text">'.$data->email.'</p>
                                    <p class="fs-12 fw-500 lh-18 text-para-text">'.$data->mobile.'</p>
                                </div>
                           </div>';
                return $userHtml;
            })
            ...
            ->addColumn('action', function ($data) {
                return '<div class="action__buttons d-flex justify-content-end"><a href="' . route('agent.ticket.view-ticket', $data->id) . '" class="p-0 bg-transparent w-30 h-30 ms-auto bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center">
                          <img src="' . asset('customer/assets/images/preview-open.png') . '" alt="" class="max-w-14">
                        </a></div>
                        ';
...
//            })
            ...
            ->addColumn('action', function ($data) {
                return '<div class="action__buttons d-flex justify-content-end"></div><a href="' . route('agent.ticket.view-ticket', $data->id) . '" class="p-0 bg-transparent w-30 h-30 ms-auto bd-one bd-c-stroke rounded-circle d-flex justify-content-center align-items-center">
                          <img src="' . asset('customer/assets/images/preview-open.png') . '" alt="" class="max-w-14">
                        </a>
            ...
            ->addColumn('ticket_id', function ($data) {
                if($data->last_reply_id == null){
                    return '<a href="'.route('agent.ticket.view-ticket', $data->id).'" class="btn btn-outline-primary position-relative ticket-tracking-id agent-ticket-id" href="">
                            '.$data->tracking_no.'
