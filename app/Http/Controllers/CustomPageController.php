        ...
        if ($request->ajax()) {
            ...
            return datatables($pages)
                ...
                ->addColumn('link', function ($data) {
                    return url('custom-page/' . htmlspecialchars($data->slug)) . '<button class="copy-custom-page-link" data-src="' . url('custom-page/' . htmlspecialchars($data->slug)) . '"><i class="fa fa-copy"></i> </button>';
                })
                ->addColumn('status', function ($data) {
                    if ($data->status == 1) {
                        return '<span class="zBadge zBadge-active">Published</span>';
                    } else {
                        return '<span class="zBadge zBadge-deactivate">Unpublished</span>';
                    }
                })
