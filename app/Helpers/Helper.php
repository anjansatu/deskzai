    ...
    function getTicketIdHtml($data)
    {
        ...
        if ($data->last_reply_id == null && $data->status == STATUS_PENDING) {
            return '<a href="' . getRoute($data) . '" class="btn btn-outline-primary position-relative ticket-tracking-id  agent-ticket-id" >
                            ' . $data->tracking_no . '
                             <span class="zBadge zBadge-new rounded-pill position-absolute start-100 top-0 translate-middle">
                                New
                             </span>
                        </a>';
            ...
            if (\auth()->user()->role == USER_ROLE_CUSTOMER) {
                return '<a href="' . getRoute($data) . '" class="btn btn-outline-primary position-relative ticket-tracking-id  agent-ticket-id" >
                            ' . $data->tracking_no . '
                              <span class="zBadge zBadge-new position-absolute rounded-pill start-100 top-0 translate-middle">
                ...
                if ($data->last_reply_by != null && getRoleByUserId($data->last_reply_by) == USER_ROLE_CUSTOMER) {
                    ...
                    return '<a href="' . getRoute($data) . '" class="btn btn-outline-primary position-relative ticket-tracking-id  agent-ticket-id" >
                            ' . $data->tracking_no . '
                            <span class="zBadge zBadge-new position-absolute rounded-pill top-0 start-100 translate-middle">
                                <i class="fa-regular fa-envelope mb-0"></i>
                             </span>
                        </a>';
                }
...
if (!function_exists('getTicketStatusHtml')) {
    function getTicketStatusHtml($data)
    {
        if ($data->status == STATUS_OPEND) {
            return '<span class="zBadge zBadge-new">New</span>';
        } else if ($data->status == STATUS_INPROGRESS) {
            return '<span class="zBadge zBadge-inProgress">In Progress</span>';
        } else if ($data->status == STATUS_CANCELED) {
            return '<span class="zBadge zBadge-cancel">Canceled</span>';
        } else if ($data->status == STATUS_ON_HOLD) {
            return '<span class="zBadge zBadge-onHold">On Hold</span>';
        } else if ($data->status == STATUS_CLOSED) {
            return '<span class="zBadge zBadge-closed">Closed</span>';
        } else if ($data->status == STATUS_RESOLVED) {
            return '<span class="zBadge zBadge-resolved">Resolved</span>';
        } else if ($data->status == STATUS_REOPEN) {
            return '<span class="zBadge zBadge-open">Re Open</span>';
        } else if ($data->status == STATUS_SUSPENDED) {
            return '<span class="zBadge zBadge-suspended">Suspended</span>';
        } else if ($data->status == 'delete') {
            return '<span class="zBadge zBadge-deleted">Deleted</span>';
        } else {
...
//        $licence = $envato?->enable_purchase_code == 1 ? '<div><span><li>' . htmlspecialchars($data->envato_licence) . ' </span></div></li>' : '';
        $ticketDetails = '<a class="fs-14 fw-500 lh-17 text-title-black" href="' . getRoute($data) . '" >
                                        ' . Str::limit(htmlspecialchars($data->ticket_title), 40, '...') . '
                           </a>
                            <ul class="tableList-items">
                            ...
                            ' . getTicketPriorityHtml($data) . '
                              ...
                              <div>' . getTicketStatusHtml($data) . '</div>';
...
if (!function_exists('getTicketAssignToHtml')) {
    function getTicketAssignToHtml($data)
    {
        ...
        if ($data->assignTo != null && count($data->assignTo) > 0) {
            $assigned_name .= '<div class="ticketUserImages">';
            foreach ($data->assignTo as $u) {
                $assigned_name .= '<img title = "' . getUserInfoById($u->assigned_to, 'name') . '(' . getUserInfoById($u->assigned_to, 'email') . ')' . '" src = "' . getFileUrl(getUserInfoById($u->assigned_to, 'image')) . '" alt = "img" >';
            }
            $assigned_name .= '</div>';
        } else {
            $assigned_name = '<p>Not Assigned</p>';
        }
